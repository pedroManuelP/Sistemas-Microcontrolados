...
void updateDisplay(uint8_t type, int16_t *gyro, float *temperatura, float* euler, float * accelF){
	...
	switch (type)
	{
		...
		hc05_load("x(g)  y(g)  z(g)");
		hc05_send_string("\r");
		hc05_load(str_accel_x);
		hc05_load(str_accel_y);
		hc05_load(str_accel_z);
		hc05_send_string("\r");
		break;
		case case_gyro:
		lcd_clear();
		...
		hc05_load("x(d)  y(d)  z(d)");
		hc05_send_string("\r");
		hc05_load(str_gyro_x);
		hc05_load("   ");
		hc05_load(str_gyro_y);
		hc05_load("   ");
		hc05_load(str_gyro_z);
		hc05_send_string("\r");
		break;
		case case_euler:
		lcd_clear();
		...
		hc05_load("x;y;z;temp:");
		hc05_load(str_temp);
		hc05_send_string("\r");
		hc05_load(str_euler_x);
		hc05_load(str_euler_y);
		hc05_load(str_euler_z);
		hc05_send_string("\r");
		break;
		default:
		break;
